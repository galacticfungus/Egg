...
impl SnapshotsState {
    ...
    pub fn change_state<F>(&mut self, atomic_updater: &mut AtomicUpdate, callback: F) -> Result<(), Error>
        where F: FnOnce(&mut StateBuilder) -> Result<(), Error>,
    {
        let mut builder = StateBuilder::new(self);
        // Get the requried changes from the caller
        callback(&mut builder).map_err(|err| err.add_generic_message("Change state callback method has returned a user error"))?;
        // Queue replacing the current state file with the new one
        let path_to_file = atomic_updater.queue_replace(self.path_to_state_file.as_path()).map_err(|err| err.add_generic_message("During a change snapshot state operation"))?;
        // Prepares the new state on disk, this wont actually replace the old state until the atomic operation completes
        SnapshotsState::update_state(path_to_file.as_path(), &self)
            .map_err(|err| err.add_generic_message("While trying to change the current snapshot state"))?;
        Ok(())
    }

    /// Writes the new state to disk, since this uses the atomic updater, the changes will not be written immediately
    fn update_state(path_to_state: &path::Path, state_to_write: &SnapshotsState) -> Result<(), Error> {
