...
impl<'a> AtomicUpdate<'a> {
    ...
    pub fn new(path_to_working: &'a path::Path, path_to_repository: &'a path::Path) -> AtomicUpdate<'a> {
        ...
        AtomicUpdate {
            path_to_create_complete: path_to_repository.join(AtomicLocation::CreateComplete.get_path()),
            path_to_create_working: path_to_repository.join(AtomicLocation::CreateWorking.get_path()),
            path_to_replace_working: path_to_repository.join(AtomicLocation::ReplaceWorking.get_path()),
            path_to_replace_complete: path_to_repository.join(AtomicLocation::ReplaceComplete.get_path()),
            path_to_replace_previous: path_to_repository.join(AtomicLocation::ReplacePrevious.get_path()),
            path_to_replace_remove: path_to_repository.join(AtomicLocation::ReplaceRemove.get_path()),
            path_to_store_working: path_to_repository.join(AtomicLocation::StoreWorking.get_path()),
            path_to_store_complete: path_to_repository.join(AtomicLocation::StoreComplete.get_path()),
            path_to_repository,
            path_to_working,
            atomic_jobs: Vec::new(),
        }
    ...
    pub fn init(path_to_repository: &'a path::Path, path_to_working: &'a path::Path) -> Result<AtomicUpdate<'a>, Error> {
        ...
        create_if_needed(path_to_repository, AtomicLocation::ReplaceWorking)?;
        create_if_needed(path_to_repository, AtomicLocation::ReplaceComplete)?;
        create_if_needed(path_to_repository, AtomicLocation::ReplacePrevious)?;
        create_if_needed(path_to_repository, AtomicLocation::ReplaceRemove)?;
        create_if_needed(path_to_repository, AtomicLocation::CreateWorking)?;
        create_if_needed(path_to_repository, AtomicLocation::CreateComplete)?;
        create_if_needed(path_to_repository, AtomicLocation::StoreWorking)?;
        create_if_needed(path_to_repository, AtomicLocation::StoreComplete)?;
...
mod tests {
    ...
    fn test_first_stage_queue_create() {
        ...
        let mut atomic = AtomicUpdate::init(repository_path.as_path(), working_path.as_path()).expect("Failed to initialize repository");
