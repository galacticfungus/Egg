    ...
    pub fn cleanup(&mut self) {
        ...
        match file_item {
            ...
            fs::remove_dir_all(&path).unwrap_or_else(|err| {
                eprintln!("Failed to cleanup the test directory {}, error was {}", path.display(), err);
            });
            eprintln!("Deleted {}", path.display());
            },
            FileItem::File(path) => {
            fs::remove_file(path.as_path()).unwrap_or_else(|err| {
                eprintln!("Failed to cleanup the test file {}, error was {}", path.display(), err);
            });
            eprintln!("Deleted {}", path.display());
            }
        }
...
mod tests {
    ...
    fn test_history() {
        ...
        panic!("Failed to create the temporary directory, error was {}", err);
        });
    ...
    fn test_file_history() {
        ...
        {
        let mut file = fs::OpenOptions::new().create(true).create_new(true).write(true).read(true).open(temp_path).unwrap_or_else(|err| {
            panic!("Failed to create the temp file, error was {}", err);
        });
        file.write_u64::<LittleEndian>(12345).unwrap_or_else(|err| {
            panic!("Failed writing test data during history test, error was {}", err);
        });
        }
        assert!(temp_path.exists());
